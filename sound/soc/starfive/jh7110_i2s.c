			    ...
			    struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		if ((config->sample_rate == 8000) && playback &&
			...
			dev_warn(dev->dev,
				 "I2S: unsupported 8000 rate with S16_LE, Stereo if use wm8960.\n");
		}
...
static int dw_i2s_resume(struct snd_soc_component *component)
{
	...
	if (dev->syscon_base) {
		if (dev->is_master) {
			...
			/*set i2sdin_sel*/
			regmap_update_bits(dev->syscon_base, dev->syscon_offset_34,
					   (0x1 << 10) | (0x1 << 14) | (0x1<<17),
					   (0x0<<10) | (0x0<<14) | (0x0<<17));
		}
	}
...
static int dw_i2srx_mst_clk_init(struct dw_i2s_dev *dev)
{
	...
	int ret = devm_clk_bulk_get(dev->dev, ARRAY_SIZE(clks), clks);

	if (ret) {
		dev_err(dev->dev, "%s: failed to get i2srx clocks\n", __func__);
		goto exit;
	}
	...
	dev->rst_i2s_apb = devm_reset_control_get_exclusive(dev->dev, "apb");
	if (IS_ERR(dev->rst_i2s_apb)) {
		dev_err(dev->dev, "failed to get apb_i2srx reset control\n");
		ret = PTR_ERR(dev->rst_i2s_apb);
		goto exit;
	}

	dev->rst_i2s_bclk = devm_reset_control_get_exclusive(dev->dev, "bclk");
	if (IS_ERR(dev->rst_i2s_bclk)) {
		dev_err(dev->dev, "failed to get i2s bclk rx reset control\n");
	...
	if (ret) {
		dev_err(dev->dev, "failed to prepare enable clk_i2srx_apb\n");
		goto exit;
	}

	ret = clk_set_parent(dev->clk_i2s_bclk, dev->clk_i2s_bclk_mst);
	if (ret) {
		dev_err(dev->dev, "failed to set parent clk_i2s_bclk\n");
		goto err_dis_bclk_mst;
	}

	ret = clk_set_parent(dev->clk_i2s_lrck, dev->clk_i2s_lrck_mst);
	if (ret) {
		dev_err(dev->dev, "failed to set parent clk_i2s_lrck\n");
		goto err_dis_bclk_mst;
	}
	...
	if (ret) {
		dev_err(dev->dev, "failed to prepare enable clk_i2srx_3ch_bclk_mst\n");
		goto err_dis_bclk_mst;
	}
...
static int dw_i2srx_clk_init(struct dw_i2s_dev *dev)
{
	static struct clk_bulk_data clks[] = {
		{ .id = "apb" },
		{ .id = "bclk_mst" },
	...
	if (ret) {
		dev_err(dev->dev, "%s: failed to get audio_subsys clocks\n", __func__);
		return ret;
	}
	...
	ret = clk_prepare_enable(dev->clk_i2s_apb);
	if (ret) {
		dev_err(dev->dev, "%s: failed to enable clk_i2s_apb\n", __func__);
		goto disable_apb_clk;
	}

	ret = clk_prepare_enable(dev->clk_i2s_bclk_mst);
	if (ret) {
		dev_err(dev->dev, "%s: failed to enable CLK_ADC_BCLK\n", __func__);
		goto disable_bclk;
	}

	dev->rst_i2s_apb = devm_reset_control_array_get_exclusive(dev->dev);
	if (IS_ERR(dev->rst_i2s_apb)) {
		dev_err(dev->dev, "%s: failed to get rsts reset control\n", __func__);
		goto disable_rst;
	}

	ret = reset_control_assert(dev->rst_i2s_apb);
	if (ret) {
		dev_err(dev->dev, "%s: failed to reset control assert rstc_rx\n", __func__);
		goto disable_rst;
	}
	udelay(5);
	ret = reset_control_deassert(dev->rst_i2s_apb);
	if (ret) {
		dev_err(dev->dev, "%s: failed to reset control deassert rstc_rx\n", __func__);
		goto disable_rst;
	}
...
static int dw_i2stx_4ch0_clk_init(struct dw_i2s_dev *dev)
{
	static struct clk_bulk_data clks[] = {
		{ .id = "bclk_mst" },
		{ .id = "lrck_mst" },
	...
	if (ret) {
		dev_err(dev->dev, "%s: failed to get i2s clocks\n", __func__);
		goto exit;
	}
	...
	dev->rst_i2s_apb = devm_reset_control_get_exclusive(dev->dev, "apb");
	if (IS_ERR(dev->rst_i2s_apb)) {
		dev_err(dev->dev, "failed to get i2s apb reset control\n");
		ret = PTR_ERR(dev->rst_i2s_apb);
		goto exit;
	}
	...
	dev->rst_i2s_bclk = devm_reset_control_get_exclusive(dev->dev, "bclk");
	if (IS_ERR(dev->rst_i2s_bclk)) {
		dev_err(dev->dev, "failed to get i2s bclk reset control\n");
		ret = PTR_ERR(dev->rst_i2s_bclk);
		goto exit;
	}
	...
	ret = clk_prepare_enable(dev->clk_i2s_apb);
	if (ret) {
		dev_err(dev->dev, "failed to prepare enable clk_i2s_apb\n");
		goto exit;
	}
	...
	if (ret) {
		dev_err(dev->dev, "failed to set parent clk_i2s_lrck_mst\n");
		goto err_dis_bclk_mst;
	}
...
static int dw_i2stx_4ch1_clk_init(struct dw_i2s_dev *dev)
{
	static struct clk_bulk_data clks[] = {
		{ .id = "bclk_mst" },
		{ .id = "lrck_mst" },
	...
	ret = clk_prepare_enable(dev->clk_i2s_bclk_mst);
	if (ret) {
		dev_err(dev->dev, "%s: failed to enable clk_i2s_bclk_mst\n", __func__);
		goto disable_bclk;
	}
	...
	ret = reset_control_assert(dev->rst_i2s_apb);
	if (ret) {
		dev_err(dev->dev, "%s: failed to reset control assert rsts\n", __func__);
		goto disable_rst;
	}
	udelay(5);
	ret = reset_control_deassert(dev->rst_i2s_apb);
	if (ret) {
		dev_err(dev->dev, "%s: failed to reset control deassert rsts\n", __func__);
		goto disable_rst;
	}
...
static int dw_i2s_probe(struct platform_device *pdev)
{
	...
	dw_i2s_dai->name = dev_name(&pdev->dev);
