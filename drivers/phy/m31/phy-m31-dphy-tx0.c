...
static void reset_dphy(struct sf_dphy *priv, int resetb)
{
	...
	top_sys_write32(priv, VID_MCTL_MAIN_PHY_CTL, precfg|cfg_ck2_ck3_ck_enable);

	precfg = top_sys_read32(priv, VID_MCTL_MAIN_DATA_CTL) & ~cfg_link_enable;
	top_sys_write32(priv, VID_MCTL_MAIN_DATA_CTL, precfg|cfg_link_enable);

	precfg = top_sys_read32(priv, SCFG_PHY_RESETB);
