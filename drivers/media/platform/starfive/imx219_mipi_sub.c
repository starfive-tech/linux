...
static int imx219_power_up(struct imx219 *imx219)
{
	...
	u16 chip_id = imx219_cam_v2_read_16(imx219->i2c_client, IMX219_REG_CHIP_ID, 0xffff);
...
{
	int i, j;
	for (i = 0; i < sizeof(imx219_mode_params_cs1)/sizeof(imx219_mode_params_cs1[0]); i++) {
		if (imx219->mode == imx219_mode_params_cs1[i].mode) {
			for (j = 0; j < imx219_mode_params_cs1[i].regval_num; j++) {
				imx219_write_reg(imx219->i2c_client,imx219_mode_params_cs1[i].regval[j].addr, imx219_mode_params_cs1[i].regval[j].val);
			}
			return 0;
		}
	}
...
int mipicam_set_mode_cs1(struct imx219 *imx219)
{
	imx219_stop(imx219);
			...
			const struct i2c_device_id *id)
{
	...
	if (mipicam_set_mode_cs1(imx219)) {
...
static __init int init_imx219rx1(void)
{ 
    int err;
	printk("init_imx219rx1 sub\n");
...
static __exit void exit_imx219rx1(void)
{
	i2c_del_driver(&imx219_i2c_driver);
}

fs_initcall(init_imx219rx1);
module_exit(exit_imx219rx1);
