...
static int sfctemp_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(sfctemp->clk_sense)) {
		...
		dev_err(dev, "error getting %s clock: %d\n", "sense", ret);
		return ret;
	}
	...
	if (IS_ERR(sfctemp->rst_sense)) {
		...
		dev_err(dev, "error getting %s reset: %d\n", "sense", ret);
		return ret;
	}
	ret = reset_control_assert(sfctemp->rst_sense);
	if (ret) {
		dev_err(dev, "error asserting %s reset: %d\n", "sense", ret);
		return ret;
	}
	...
	if (ret) {
		dev_err(dev, "error enabling temperature sensor: %d\n", ret);
		return ret;
	}
