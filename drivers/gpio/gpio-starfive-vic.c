...
struct starfive_gpio {
	...
	struct starfive_gpio	*self_ptr[MAX_GPIO];
};
...
static int starfive_gpio_probe(struct platform_device *pdev)
{
	...
	ret = gpiochip_irqchip_add(&chip->gc, &starfive_irqchip, 0,
...
#if 0
	printk("this is debug by lophyel %s %s %d\n",__FILE__, __func__, __LINE__);
	...
	for (gpio = 0; gpio < ngpio; ++gpio) {
		...
		irq_set_chained_handler_and_data(irq, starfive_irq_handler, &chip->self_ptr[gpio]);
