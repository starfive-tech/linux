...
static inline void osl_cache_flush(void *virt_start, unsigned long len)
{
	unsigned long start = dw_virt_to_phys(virt_start);
	unsigned long addr = _ALIGN_DOWN(start, 64);
	unsigned long lenth = len+(start&63);

	printk("this is debug for flush cache %s %d\n",__func__,__LINE__);
	starfive_flush_dcache(addr,lenth);
}
	#define OSL_CACHE_FLUSH(va, len)	osl_cache_flush((void *)(va), len)
	#define OSL_CACHE_INV(va, len)		OSL_CACHE_FLUSH((void *)(va), len)
...
static inline void osl_cache_flush(void *virt_start, unsigned long len)
{
	...
	#define OSL_CACHE_FLUSH(va, len)	osl_cache_flush(va, len)
        #define OSL_CACHE_INV(va, len)		osl_cache_flush(va, len)
