...
static void *starfive_eqos_probe(struct platform_device *pdev,
			       struct plat_stmmacenet_data *data,
			       struct stmmac_resources *res)
{
	struct device *dev = &pdev->dev;
	...
	eqos->stmmac_ahb_rst = devm_reset_control_get(&pdev->dev, "ahb");
	if (IS_ERR(eqos->stmmac_ahb_rst)) {
		err = PTR_ERR(eqos->clk_gtxc);
		goto disable_gtxc;
	}

	err = reset_control_deassert(eqos->stmmac_ahb_rst);
	if (err < 0)
		goto disable_gtxc;
...
disable_gtxc:
	clk_disable_unprepare(eqos->clk_gtxc);
disable_gtx:
...
static int starfive_eqos_remove(struct platform_device *pdev)
{
	...
	reset_control_assert(eqos->stmmac_ahb_rst);
	clk_disable_unprepare(eqos->clk_tx);
	clk_disable_unprepare(eqos->clk_gtx);
	clk_disable_unprepare(eqos->clk_gtxc);
